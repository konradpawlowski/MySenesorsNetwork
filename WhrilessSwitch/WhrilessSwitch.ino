...
#define pLED_BLUE   10//4
#define pRELAY_1    5 //PWM
#define pRELAY_2    6 //PWM
#define pNRF_CE     7
#define pNRF_CSN    8
#define pLED_RED    9//9 //PWM
#define pLED_GREEN 4//4//10 //PWM
#define pFLASH_SS  A0
#define pDS18B20   A1
...
void loop()
{
	...
	if (tBtn1Released1 > 0 && millis() - tBtn1Released1 > tWaitTwoPress && tBtn1Released2 == 0) //One pulsation
	{
		...
		else if (tBtn1Released1 - tBtnPressed1 > tShortPress) //Long press
		{
			...
			send(msgR1.set(status), ACK);
			send(msgR2.set(status), ACK);
		}
	}
	else if (tBtn1Released1 > 0 && tBtn1Released2 > 0) //Double press
	{
		sBtnPressed = true;
...
void receive(const MyMessage &message)
{
	...
	{
		digitalWrite((message.sensor == MS_RELAY1_CHILD_ID ? pRELAY_1 : pRELAY_2), message.getBool());
		refreshLedStatus();
	}
	else if (message.type == V_PERCENTAGE)
	{
		brightnessLed = map(atoi(message.data), 0, 100, 0, 1023);
	}
}
